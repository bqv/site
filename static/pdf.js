...
(function pdfjsWrapper() {
  ...
  var Util = PDFJS.Util = function UtilClosure() {
    ...
    Util.loadScript = function Util_loadScript(src, callback) {
      ...
      var loaded = false;
      Polymer.dom(script).setAttribute('src', src);
      if (callback) {
      ...
      Polymer.dom(document.getElementsByTagName('head')[0]).appendChild(script);
    };
    return Util;
  }();
  /**
  ...
  */
    ...
    Metadata.prototype = {
      parse: function Metadata_parse() {
        ...
        if (rdf.nodeName.toLowerCase() !== 'rdf:rdf') {
          // Wrapped in <xmpmeta>
          rdf = Polymer.dom(rdf).firstChild;
          while (rdf && rdf.nodeName.toLowerCase() !== 'rdf:rdf') {
            rdf = Polymer.dom(rdf).nextSibling;
          }
        }
        ...
        var children = Polymer.dom(rdf).childNodes, desc, entry, name, i, ii, length, iLength;
        for (i = 0, length = children.length; i < length; i++) {
          desc = children[i];
          if (desc.nodeName.toLowerCase() !== 'rdf:description') {
            continue;
          }
          for (ii = 0, iLength = Polymer.dom(desc).childNodes.length; ii < iLength; ii++) {
            if (Polymer.dom(desc).childNodes[ii].nodeName.toLowerCase() !== '#text') {
              entry = Polymer.dom(desc).childNodes[ii];
              name = entry.nodeName.toLowerCase();
              this.metadata[name] = Polymer.dom(entry).textContent.trim();
            }
          }
        }
      },
    ...
    CanvasGraphics.prototype = {
      ...
      getColorN_Pattern: function CanvasGraphics_getColorN_Pattern(IR) {
        ...
        if (IR[0] === 'TilingPattern') {
          ...
          pattern = new TilingPattern(IR, color, this.ctx, this.objs, this.commonObjs, baseTransform);
        } else {
  ...
  FontLoader.prototype = {
    insertRule: function fontLoaderInsertRule(rule) {
      ...
      if (!styleElement) {
        ...
        Polymer.dom(document.documentElement.getElementsByTagName('head')[0]).appendChild(styleElement);
      }
    ...
    clear: function fontLoaderClear() {
      var styleElement = this.styleElement;
      if (styleElement) {
        Polymer.dom(Polymer.dom(styleElement).parentNode).removeChild(styleElement);
    ...
    prepareFontLoadEvent: function fontLoaderPrepareFontLoadEvent(rules, fonts, request) {
      ...
      Polymer.dom(div).setAttribute('style', 'visibility: hidden;' + 'width: 10px; height: 10px;' + 'position: absolute; top: 0px; left: 0px;');
      for (i = 0, ii = names.length; i < ii; ++i) {
        var span = document.createElement('span');
        Polymer.dom(span).textContent = 'Hi';
  ...
  var AnnotationLayer = function AnnotationLayerClosure() {
    ...
    function getContainer(data, page, viewport) {
      ...
      Polymer.dom(container).setAttribute('data-annotation-id', data.id);
      ...
      CustomStyle.setProp('transformOrigin', container, -data.rect[0] + 'px ' + -data.rect[1] + 'px');
    ...
    function getHtmlElementForTextWidgetAnnotation(item, page) {
      ...
      Polymer.dom(content).textContent = item.fieldValue;
      ...
      var fontObj = item.fontRefName ? page.commonObjs.getData(item.fontRefName) : null;
      setTextStyles(content, item, fontObj);
      Polymer.dom(element).appendChild(content);
      return element;
    }
    function getHtmlElementForTextAnnotation(item, page, viewport) {
      ...
      Polymer.dom(content).setAttribute('hidden', true);
      var i, ii;
      ...
      Polymer.dom(title).textContent = item.title;
      if (!item.content && !item.title) {
        ...
        for (i = 0, ii = lines.length; i < ii; ++i) {
          var line = lines[i];
          Polymer.dom(e).appendChild(document.createTextNode(line));
          if (i < ii - 1) {
            Polymer.dom(e).appendChild(document.createElement('br'));
          }
        }
        Polymer.dom(text).appendChild(e);
        var pinned = false;
        var showAnnotation = function showAnnotation(pin) {
          ...
          if (content.hasAttribute('hidden')) {
            ...
            Polymer.dom(content).removeAttribute('hidden');
          }
        };
      ...
      Polymer.dom(content).appendChild(title);
      Polymer.dom(content).appendChild(text);
      Polymer.dom(contentWrapper).appendChild(content);
      Polymer.dom(container).appendChild(image);
      Polymer.dom(container).appendChild(contentWrapper);
      return container;
    }
    function getHtmlElementForLinkAnnotation(item, page, viewport, linkService) {
      ...
      Polymer.dom(container).appendChild(link);
      return container;
    }
    function getHtmlElement(data, page, viewport, linkService) {
      ...
      case AnnotationType.LINK:
        return getHtmlElementForLinkAnnotation(data, page, viewport, linkService);
      default:
    ...
    function render(viewport, div, annotations, page, linkService) {
      for (var i = 0, ii = annotations.length; i < ii; i++) {
        ...
        Polymer.dom(div).appendChild(element);
      }
    }
    function update(viewport, div, annotations) {
      for (var i = 0, ii = annotations.length; i < ii; i++) {
        var data = annotations[i];
        var element = Polymer.dom(div).querySelector('[data-annotation-id="' + data.id + '"]');
        if (element) {
          CustomStyle.setProp('transform', element, 'matrix(' + viewport.transform.join(',') + ')');
        }
      }
      Polymer.dom(div).removeAttribute('hidden');
    }
    return {
      render: render,
      update: update
  ...
  var renderTextLayer = function renderTextLayerClosure() {
    ...
    function appendText(textDivs, viewport, geom, styles) {
      ...
      Polymer.dom(textDiv).textContent = geom.str;
    ...
    function render(task) {
      ...
      for (var i = 0; i < textDivsLength; i++) {
        ...
        var width = ctx.measureText(Polymer.dom(textDiv).textContent).width;
        if (width > 0) {
          Polymer.dom(textLayerFrag).appendChild(textDiv);
          var transform;
    ...
    function renderTextLayer(renderParameters) {
      var task = new TextLayerRenderTask(Polymer.dom(renderParameters).textContent, renderParameters.container, renderParameters.viewport, renderParameters.textDivs);
  ...
  var SVGGraphics = function SVGGraphicsClosure() {
    ...
    SVGGraphics.prototype = {
      ...
      restore: function SVGGraphics_restore() {
        ...
        Polymer.dom(this.pgrp).appendChild(this.tgrp);
      },
      ...
      getSVG: function SVGGraphics_getSVG(operatorList, viewport) {
        ...
        return this.loadDependencies(operatorList).then(function () {
          ...
          Polymer.dom(this.pgrp).appendChild(this.defs);
          Polymer.dom(this.pgrp).appendChild(this.tgrp);
          Polymer.dom(this.svg).appendChild(this.pgrp);
      ...
      setTextMatrix: function SVGGraphics_setTextMatrix(a, b, c, d, e, f) {
        ...
        Polymer.dom(current.txtElement).appendChild(current.tspan);
      },
      ...
      showText: function SVGGraphics_showText(glyphs) {
        ...
        for (i = 0; i < glyphsLength; ++i) {
          ...
          x += charWidth;
          Polymer.dom(current.tspan).textContent += character;
        }
        if (vertical) {
          current.y -= x * textHScale;
        } else {
        ...
        Polymer.dom(current.txtElement).appendChild(current.tspan);
        Polymer.dom(current.txtgrp).appendChild(current.txtElement);
        Polymer.dom(this.tgrp).appendChild(current.txtElement);
      },
      ...
      addFontStyle: function SVGGraphics_addFontStyle(fontObj) {
        if (!this.cssStyle) {
          ...
          Polymer.dom(this.defs).appendChild(this.cssStyle);
        }
        var url = PDFJS.createObjectURL(fontObj.data, fontObj.mimetype);
        Polymer.dom(this.cssStyle).textContent += '@font-face { font-family: "' + fontObj.loadedName + '";' + ' src: url(' + url + '); }\n';
      },
      ...
      endText: function SVGGraphics_endText() {
        if (this.current.pendingClip) {
          Polymer.dom(this.cgrp).appendChild(this.tgrp);
          Polymer.dom(this.pgrp).appendChild(this.cgrp);
        } else {
          Polymer.dom(this.pgrp).appendChild(this.tgrp);
        }
        this.tgrp = document.createElementNS(NS, 'svg:g');
      ...
      constructPath: function SVGGraphics_constructPath(ops, args) {
        ...
        Polymer.dom(this.tgrp).appendChild(current.path);
      ...
      clip: function SVGGraphics_clip(type) {
        ...
        Polymer.dom(this.clippath).appendChild(clipElement);
        Polymer.dom(this.defs).appendChild(this.clippath);
      ...
      paintSolidColorImageMask: function SVGGraphics_paintSolidColorImageMask() {
        ...
        Polymer.dom(this.tgrp).appendChild(rect);
      },
      paintJpegXObject: function SVGGraphics_paintJpegXObject(objId, w, h) {
        ...
        Polymer.dom(this.tgrp).appendChild(imgEl);
        if (current.pendingClip) {
          Polymer.dom(this.cgrp).appendChild(this.tgrp);
          Polymer.dom(this.pgrp).appendChild(this.cgrp);
        } else {
      ...
      paintInlineImageXObject: function SVGGraphics_paintInlineImageXObject(imgData, mask) {
        ...
        if (mask) {
          Polymer.dom(mask).appendChild(imgEl);
        } else {
          Polymer.dom(this.tgrp).appendChild(imgEl);
        }
        if (current.pendingClip) {
      ...
      paintImageMaskXObject: function SVGGraphics_paintImageMaskXObject(imgData) {
        ...
        Polymer.dom(this.defs).appendChild(mask);
        Polymer.dom(this.tgrp).appendChild(rect);
