...
(function checkWindowAtobCompatibility() {
  ...
  window.atob = function (input) {
    ...
    for (// initialize result and counters
      ...
      buffer = input.charAt(idx++); // character found in table?
...
(function checkOnClickCompatibility() {
  ...
  function isDisabled(node) {
    return node.disabled || Polymer.dom(node).parentNode && isDisabled(Polymer.dom(node).parentNode);
  }
...
(function checkNavigatorLanguage() {
  if ('language' in navigator && /^[a-z]+(-[A-Z]+)?$/.test(navigator.language)) {
    return;
  }
